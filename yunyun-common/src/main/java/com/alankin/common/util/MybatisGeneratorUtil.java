    ...
    public static void generator(
        ...
        try {
            ...
            for (Map map : result) {
                ...
                String model_name = lineToHump(ObjectUtils.toString(map.get("TABLE_NAME")));
                table.put("model_name", model_name);
                tables.add(table);

                File mapperxml = new File(targetProject+ "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/dao/mapper/"+model_name + "Mapper.xml");
                if (mapperxml.exists()) {
                    deleteDir(mapperxml);
                }
            }
...
//            deleteDir(new File(targetProjectSqlMap + "/src/main/java/" + packageName.replaceAll("\\.", "/") + "/dao/mapper"));
        ...
        System.out.println("========== 开始运行MybatisGenerator 生成实体和mapper及mapper.xml ==========");
        ...
        for (int i = 0; i < tables.size(); i++) {
            ...
            if (serviceMockFile.exists()) {
                deleteDir(serviceMockFile);
            }
            VelocityContext context1 = new VelocityContext();
            context1.put("package_name", packageName);
            context1.put("model", model);
            context1.put("ctime", ctime);
            VelocityUtil.generate(serviceMock_vm, serviceMock, context1);
            System.out.println(serviceMock);
            // 生成serviceImpl
            File serviceImplFile = new File(serviceImpl);
            if (serviceImplFile.exists()) {
                deleteDir(serviceImplFile);
            }
            VelocityContext context2 = new VelocityContext();
            context2.put("package_name", packageName);
            context2.put("model", model);
            context2.put("mapper", StringUtil.toLowerCaseFirstOne(model));
            context2.put("ctime", ctime);
            VelocityUtil.generate(serviceImpl_vm, serviceImpl, context2);
     ...
     * @throws Exception
     */
    public static void generatorByAppoint(
            String jdbcDriver,
            String jdbcUrl,
            String jdbcUsername,
            String jdbcPassword,
        ...
        if (tadbleNames != null || tadbleNames.length > 0) {
            StringBuffer stringBuffer = new StringBuffer();
            for (int i = 0; i < tadbleNames.length; i++) {
                if (i == 0) {
                    stringBuffer.append(" ( ");
                }
                if (i != tadbleNames.length - 1) {
                    stringBuffer.append(" table_name ='" + tadbleNames[i] + "' OR ");
                } else {
                    stringBuffer.append(" table_name ='" + tadbleNames[i] + "'");
                }
                if (i == tadbleNames.length - 1) {
                    stringBuffer.append(" );");
                }
