...
public abstract class BaseServiceImpl<Mapper, Record, Example> implements BaseService<Record, Example> {
     ...
     * @param record
     */
    private void injectParam(Record record) {
        try {
            Class<?> recordClass = record.getClass();
            Method getUid = recordClass.getDeclaredMethod("getUid");
            Long aLong = (Long) getUid.invoke(record);
            if (aLong == null || aLong == 0) {
                Method setUid = recordClass.getDeclaredMethod("setUid", Long.class);
                setUid.invoke(record, new SnowflakeIdWorker(0, 0).nextId());
            }
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
        ...
        try {
            Class<?> recordClass = record.getClass();
            Method getCreateTime = recordClass.getDeclaredMethod("getCreateTime");
            Integer anInt = (Integer) getCreateTime.invoke(record);
            if (anInt == null || anInt == 0) {
                Method setCreateTime = recordClass.getDeclaredMethod("setCreateTime", Integer.class);
                setCreateTime.invoke(record, (int) (SystemClock.now() / 1000));
            }
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
     ...
     * @param record
     */
    private void injectUpdateParam(Record record) {
        try {
            Class<?> recordClass = record.getClass();
            Method getCreateTime = recordClass.getDeclaredMethod("getUpdateTime");
            Integer anInt = (Integer) getCreateTime.invoke(record);
            if (anInt == null || anInt == 0) {
                Method setCreateTime = recordClass.getDeclaredMethod("setUpdateTime", Integer.class);
                setCreateTime.invoke(record, (int) (SystemClock.now() / 1000));
            }
        } catch (NoSuchMethodException e) {
            e.printStackTrace();
    ...
    public PageInfo<Record> selectByExampleForStartPage(Example example, Integer pageNum, Integer pageSize) {
        return this.selectByExampleForStartPage(example, pageNum, pageSize, true);
    }
    ...
    public PageInfo<Record> selectByExampleForStartPage(Example example, Integer pageNum, Integer pageSize, boolean count) {
        try {
            ...
            PageInfo<Record> info = new PageInfo<>(page.getResult());
            return info;
        } catch (IllegalAccessException e) {
            e.printStackTrace();
        } catch (InvocationTargetException e) {
    ...
    public PageInfo selectForStartPageByMethod(String method, Object param, Integer pageNum, Integer pageSize, boolean count) {
        try {
            ...
            if (param == null) {
                ...
                Page<Object> page = PageHelper.startPage(pageNum, pageSize, count);
                selectByExample.invoke(mapper);
                info = new PageInfo<>(page.getResult());
            } else {
                selectByExample = mapper.getClass().getDeclaredMethod(method, Object.class);
                Page<Object> page = PageHelper.startPage(pageNum, pageSize, count);
                selectByExample.invoke(mapper, param);
                info = new PageInfo<>(page.getResult());
            }
            return info;
        } catch (IllegalAccessException e) {
    ...
    public PageInfo selectForStartPageByMethod(String method, Object param, Integer pageNum, Integer pageSize) {
        return this.selectForStartPageByMethod(method, param, pageNum, pageSize, true);
    }

    @Override
    public PageInfo selectForStartPageByMethod(String method, Integer pageNum, Integer pageSize) {
        return this.selectForStartPageByMethod(method, null, pageNum, pageSize, true);
    }

    @Override
    public PageInfo selectForStartPageByMethod(String method, Integer pageNum, Integer pageSize, boolean count) {
        return this.selectForStartPageByMethod(method, null, pageNum, pageSize, count);
    }
